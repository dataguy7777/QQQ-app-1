...
TICKER_LEV  = "QQQ3.MI"    # 3× leveraged ETF (Borsa Italiana)


@lru_cache(maxsize=8)
def _download(symbol: str, start: str) -> pd.DataFrame:
    """
    Returns a tz-naive price column for *symbol*.
    """
    ...
    if df.index.tz is not None:               # strip timezone
...
def build_dataset(start: str) -> pd.DataFrame:
    """
    QQQ, QQQ3.MI and synthetic ``QQQ×3`` merged on common dates.
    """
...
def normalise(df: pd.DataFrame) -> pd.DataFrame:
    """
    Re-base each series so the first value = 100.
    """
...
def main() -> None:
    ...
    # -------- Altair chart (fixed) -------------------------------------------
    temp = df_plot.copy()
    temp.index.name = "Date"                       # guarantee the column name
    chart_df = temp.reset_index().melt(
    ...
    st.caption(
        "**QQQ×3** is a simple 3× multiple of QQQ (no daily compounding); "
